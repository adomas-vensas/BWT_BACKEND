...
def get_kernels(x_markers, y_markers, x, y, kernel_func):
    ...
    x_lat3 = x[jnp.newaxis, ...]  # or jnp.expand_dims(x_lattice, 0)
    y_lat3 = y[jnp.newaxis, ...]

    # lift markers to [N_MARKER, 1, 1]
    x_m3 = x_markers[:, jnp.newaxis, jnp.newaxis]
    y_m3 = y_markers[:, jnp.newaxis, jnp.newaxis]

    kx = kernel_func(x_lat3 - x_m3)  # shape (N_MARKER, NX, NY)
    ky = kernel_func(y_lat3 - y_m3)
...
def multi_direct_forcing(u_init, x, y, v_markers, x_markers, y_markers, 
    ...
    u = u_init
    g = jnp.zeros_like(u)
    h = jnp.zeros((n_markers, 2), dtype=u.dtype)

    # precompute kernels: shape (N_MARKER, NX, NY)
    kernels = get_kernels(x_markers, y_markers, x, y, kernel_func)

    for _ in range(n_iter):
        # 1) interpolate u to markers:
        #    u has shape (2, NX, NY)
        #    kernels has shape (N, NX, NY)
        # -> u_prod shape (2, N, NX, NY)
        u_prod = u[:, None, :, :] * kernels[None, :, :, :]

        # sum over spatial axes -> (2, N) then transpose -> (N, 2)
        u_sum = jnp.sum(u_prod, axis=(2, 3)).T

        # 2) marker force
        delta_u_markers = v_markers - u_sum     # (N, 2)
        h = h - delta_u_markers * seg_len_markers

        # 3) spread force back to fluid:
        #    delta_u_markers^T -> (2, N)
        #    expand to (2, N, 1, 1), * kernels -> (2, N, NX, NY)
        spread = (delta_u_markers.T[:, :, None, None]
                  * kernels[None, :, :, :])

        # sum over markers axis -> (2, NX, NY)
        delta_u_field = jnp.sum(spread, axis=1) * seg_len_markers
...
def get_markers_coords_2dof(x_markers_init, y_markers_init, d):
    ...
    xMarkers = jnp.add(x_markers_init, dx)
    yMarkers = jnp.add(y_markers_init, dy)
