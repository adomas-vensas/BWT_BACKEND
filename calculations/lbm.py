...
def streaming(f):
    ...
    for d in range(f.shape[0]):
        p = f[d]
        # x‐shift (axis=0 of the 2D plane)
        if d in RIGHT:
            p = roll(p, shift=+1, axis=0)
        elif d in LEFT:
            p = roll(p, shift=-1, axis=0)
        # y‐shift (axis=1 of the 2D plane)
        if d in UP:
            p = roll(p, shift=+1, axis=1)
        elif d in DOWN:
            p = roll(p, shift=-1, axis=1)
        planes.append(p)
...
def roll(x: jnp.ndarray, shift: int, axis: int) -> jnp.ndarray:
    """
    Roll a tensor `x` along `axis` by `shift` positions, wrapping around.
    Works for any nd-array; negative shifts and shifts > dim are supported.
    """
    dim = x.shape[axis]
    # normalize shift into [0, dim)
    s = shift % dim
    if s == 0:
        return x
    # build indices [0,1,2,...,dim-1], then shift them
    idx = (jnp.arange(dim) - s) % dim  # shape (dim,)
    # use jnp.take to gather along the given axis
...
def get_macroscopic(f):
    ...
    right_sum = jnp.take(f, RIGHT_DIRS, axis=0).sum(axis=0)
    left_sum = jnp.take(f, LEFT_DIRS, axis=0).sum(axis=0)
    up_sum = jnp.take(f, UP_DIRS, axis=0).sum(axis=0)
    down_sum = jnp.take(f, DOWN_DIRS, axis=0).sum(axis=0)

    u0 = jnp.subtract(right_sum, left_sum) / rho
    u1 = jnp.subtract(up_sum, down_sum) / rho
...
def get_equilibrium(rho, u):
    ...
    ex = VELOCITIES[:, 0].reshape((9, 1, 1))
    ey = VELOCITIES[:, 1].reshape((9, 1, 1))

    u0 = u[0:1, ...]  # same as u[None,0,:,:]
    u1 = u[1:2, ...]
    ...
    rho_b = jnp.expand_dims(rho,0).repeat(9, axis=0)

    w9 = WEIGHTS.reshape((9, 1, 1))

    term = 1 + 3 * uc + 4.5 * uc**2 - 1.5 * u2
    feq = rho_b * w9 * term
...
def get_discretized_force(g, u):
    ...
    ux = u[0:1, :, :]  # shape (1, NX, NY)
    uy = u[1:2, :, :]  # shape (1, NX, NY)

    ex = VELOCITIES[:, 0].reshape((9, 1, 1))
    ey = VELOCITIES[:, 1].reshape((9, 1, 1))

    uc = ex * ux + ey * uy  # broadcasts to (9, NX, NY)

    g0 = g[0:1, :, :]
    g1 = g[1:2, :, :]

    A = ((ex - ux) * 3 + (uc * ex) * 9) * g0

    B = ((ey - uy) * 3 + (uc * ey) * 9) * g1
