...
def collision(f, feq, left_matrix):
    ...
    delta = jnp.subtract(feq, f)  # shape (9, NX, NY)

    d0, d1, d2 = delta.shape
    flat_delta = delta.reshape((d0, d1 * d2))  # (9, NX*NY)

    collided_flat = jnp.matmul(left_matrix, flat_delta);

    collided = collided_flat.reshape((d0, d1, d2))  # (9, NX, NY)

    return jnp.add(f, collided)
...
def get_source(forcing, left_matrix):
    ...
    d0, d1, d2 = forcing.shape
    flatF = jnp.reshape(forcing, [d0, d1 * d2])
    flatSrc = jnp.matmul(left_matrix, flatF)

    return jnp.reshape(flatSrc, [d0, d1, d2])
