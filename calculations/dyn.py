...
def newmark_2dof(a, v, d, h, m, k, c, dt=1, gamma=0.5, beta=0.25):
    ...
    v1 = jnp.add(v, jnp.multiply(a, dt * (1 - gamma)))

    v2 = jnp.add(d, jnp.multiply(v, dt))
    v2 = jnp.add(v2, jnp.multiply(a, dt * dt * (0.5 - beta)))

    denom = m + c1 * c  + c2 * k

    aNext = jnp.subtract(h, jnp.multiply(v1, c))
    aNext = jnp.subtract(aNext, jnp.multiply(v2, k))
    aNext = jnp.divide(aNext, denom)

    vNext = jnp.add(v1, jnp.multiply(aNext, c1))
    dNext = jnp.add(v2, jnp.multiply(aNext, c2))
